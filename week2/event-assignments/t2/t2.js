...
// Funktio, joka järjestää ravintolat aakkosjärjestykseen
function sortRestaurants() {
  return restaurants.sort((a, b) => a.name.localeCompare(b.name));
}

// Funktio, joka luo taulun ja lisää ravintolat siihen
function renderRestaurants() {
  const table = document.querySelector('table');
  const sortedRestaurants = sortRestaurants();

  // Tyhjennetään taulu ennen uudelleen täyttämistä
  table.innerHTML = `
    <tr>
      <th>Name</th>
      <th>Address</th>
  ...
  sortedRestaurants.forEach(restaurant => {
    ...
    `;
    row.addEventListener('click', () => showModal(restaurant)); // Klikkaus tuo modalin esiin
    table.appendChild(row);
  });
}

// Funktio näyttää modal-ikkunan ravintolan tiedoilla
function showModal(restaurant) {
  ...
  `;
  dialog.showModal();
  highlightRestaurant(restaurant); // Korostetaan klikattu ravintola
}

// Funktio sulkee modal-ikkunan
function closeModal() {
  const dialog = document.querySelector('dialog');
  dialog.close();
}

// Funktio, joka korostaa valitun ravintolan
function highlightRestaurant(restaurant) {
  const rows = document.querySelectorAll('table tr');
  
  // Poistetaan highlight kaikista riveistä
  rows.forEach(row => row.classList.remove('highlight'));

  // Korostetaan vain valittu ravintola
  const selectedRow = [...rows].find(row => row.querySelector('.name').textContent === restaurant.name);
  if (selectedRow) {
